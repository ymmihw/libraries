...
import static com.ymmihw.libraries.rsocket.support.Constants.SHOT_COUNT;
...
public class GameController implements Publisher<Payload> {
  ...
  private List<Long> generateShotList() {
    return Flux.range(1, SHOT_COUNT).map(x -> (long) Math.ceil(Math.random() * 1000)).collectList()
        .block();
  }
