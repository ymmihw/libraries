...
public class NegativeMatchingRulesUnitTest {
  ...
  public void givenDictionaryRules_whenValidatePassword_thenFoundIllegalWordsFromDictionary() {
    ...
    PasswordValidator passwordValidator =
        new PasswordValidator(dictionaryRule, dictionarySubstringRule);
  ...
  public void givenHistoryRule_whenValidatePassword_thenFoundIllegalWordsFromHistory() {
    ...
    passwordData.setPasswordReferences(new PasswordData.HistoricalReference("12345"),
        new PasswordData.HistoricalReference("1234"), new PasswordData.HistoricalReference("123"));
  ...
  public void givenSeveralIllegalRules_whenValidatePassword_thenFoundSeveralIllegalPatterns() {
    ...
    IllegalSequenceRule illegalSequenceRule =
    ...
    PasswordValidator passwordValidator = new PasswordValidator(illegalCharacterRule,
        illegalRegexRule, illegalSequenceRule, numberRangeRule, whitespaceRule);

    RuleResult validate = passwordValidator.validate(new PasswordData("abcd22 "));

    assertFalse(validate.isValid());
    assertEquals("ILLEGAL_CHAR:{illegalCharacter=a, matchBehavior=contains}",
        getDetail(validate, 0));
    assertEquals("ILLEGAL_MATCH:{match=cd22, pattern=\\w{2}\\d{2}}", getDetail(validate, 1));
    assertEquals("ILLEGAL_ALPHABETICAL_SEQUENCE:{sequence=abcd}", getDetail(validate, 2));
    assertEquals("ILLEGAL_NUMBER_RANGE:{number=2, matchBehavior=contains}", getDetail(validate, 3));
    assertEquals("ILLEGAL_WHITESPACE:{whitespaceCharacter= , matchBehavior=contains}",
  ...
  public void givenRepeatCharacterRegexRuleRule_whenValidatePassword_thenFoundIllegalPatternMatches() {
    ...
    assertFalse(validate.isValid());
    assertEquals("ILLEGAL_MATCH:{match=aaa, pattern=([^\\x00-\\x1F])\\1{2}}",
        getDetail(validate, 0));
    assertEquals("ILLEGAL_MATCH:{match=bbb, pattern=([^\\x00-\\x1F])\\1{2}}",
  ...
  public void givenUserNameRule_whenValidatePassword_thenFoundUserNameInPassword() {
    ...
    assertFalse(validate.isValid());
    assertEquals("ILLEGAL_USERNAME:{username=testuser, matchBehavior=contains}",
        getDetail(validate, 0));
  }

  @Test
  public void givenPasswordAndHashBeanAndEncryptedReferences_whenValidate_thenPasswordValidationShouldPass() {
    List<PasswordData.Reference> historicalReferences =
        Arrays.asList(new PasswordData.HistoricalReference("SHA256",
    ...
    PasswordValidator passwordValidator =
        new PasswordValidator(new DigestHistoryRule(encodingHashBean));
