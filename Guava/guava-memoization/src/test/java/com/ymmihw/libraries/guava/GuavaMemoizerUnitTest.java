...
public class GuavaMemoizerUnitTest {
  ...
  public void givenInteger_whenGetFactorial_thenShouldCalculateFactorial() {
    ...
    // then
    BigInteger expectedFactorial = new BigInteger(
  ...
  public void givenMemoizedSupplierWithExpiration_whenGet_thenSubsequentGetsBeforeExpiredAreFast()
    ...
    Supplier<BigInteger> memoizedSupplier;
    memoizedSupplier =
        Suppliers.memoizeWithExpiration(CostlySupplier::generateBigNumber, 3, TimeUnit.SECONDS);
  ...
  private <T> void assertSupplierGetExecutionResultAndDuration(Supplier<T> supplier,
      T expectedValue, double expectedDurationInMs) {
