...
public class CommodityClient {
  ...
  public void getBidirectionalCommodityPriceLists() throws InterruptedException {
    ...
    logger.info(
        "#######START EXAMPLE#######: BidirectionalStreaming - getCommodityPriceLists from list of commodities");
    final CountDownLatch finishLatch = new CountDownLatch(1);
    StreamObserver<StreamingCommodityQuote> responseObserver =
        new StreamObserver<StreamingCommodityQuote>() {
          @Override
          public void onNext(StreamingCommodityQuote streamingCommodityQuote) {
            ...
            switch (streamingCommodityQuote.getMessageCase()) {
              case COMODITY_QUOTE:
                ...
                logger.info(
                    "RESPONSE producer:"
                        + commodityQuote.getCommodityName()
                        + " price:"
                        + commodityQuote.getPrice());
                break;
              case STATUS:
    ...
    StreamObserver<Commodity> requestObserver =
        nonBlockingStub.bidirectionalListOfPrices(responseObserver);
    try {
      for (int i = 1; i <= 2; i++) {
        Commodity request =
