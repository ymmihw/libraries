...
import com.baeldung.grpc.errorhandling.StreamingCommodityQuote;
...
public class CommodityServer {
  ...
  public CommodityServer(int port) throws IOException {
    this.port = port;
    server = ServerBuilder.forPort(port).addService(new CommodityService()).build();
  }
  ...
  public static class CommodityService
      extends CommodityPriceProviderGrpc.CommodityPriceProviderImplBase {

    @Override
    public void getBestCommodityPrice(
        Commodity request, StreamObserver<CommodityQuote> responseObserver) {
      ...
      if (commodityLookupBasePrice.get(request.getCommodityName()) == null) {
        ...
        CommodityQuote commodityQuote =
            CommodityQuote.newBuilder()
    ...
    @Override
    public StreamObserver<Commodity> bidirectionalListOfPrices(
        StreamObserver<StreamingCommodityQuote> responseObserver) {
                    ...
                    .addDetails(
                        Any.pack(
                            ErrorInfo.newBuilder()
                                ...
                                .build()))
                    .build();
            StreamingCommodityQuote streamingCommodityQuote =
                StreamingCommodityQuote.newBuilder().setStatus(status).build();
            ...
            for (int i = 1; i <= 5; i++) {
              CommodityQuote commodityQuote =
                  CommodityQuote.newBuilder()
                      ...
                      .build();
              StreamingCommodityQuote streamingCommodityQuote =
                  StreamingCommodityQuote.newBuilder().setComodityQuote(commodityQuote).build();
              responseObserver.onNext(streamingCommodityQuote);
            }
          }
        }
  ...
  private static double fetchBestPriceBid(Commodity commodity) {
    ...
    return commodityLookupBasePrice.get(commodity.getCommodityName())
