...
import com.ymmihw.libraries.StockQuoteProviderGrpc.StockQuoteProviderBlockingStub;
import com.ymmihw.libraries.StockQuoteProviderGrpc.StockQuoteProviderStub;
...
public class StockClient {
  ...
  public void serverSideStreamingListOfStockPrices() {
    ...
    logger.info(
        "######START EXAMPLE######: ServerSideStreaming - list of Stock prices from a given stock");
    Stock request =
        Stock.newBuilder()
            .setTickerSymbol("AU")
  ...
  public void clientSideStreamingGetStatisticsOfStocks() throws InterruptedException {
    ...
    logger.info(
        "######START EXAMPLE######: ClientSideStreaming - getStatisticsOfStocks from a list of stocks");
    ...
    StreamObserver<StockQuote> responseObserver =
        new StreamObserver<StockQuote>() {
          ...
          public void onNext(StockQuote summary) {
            logger.info(
                "RESPONSE, got stock statistics - Average Price: {}, description: {}",
                summary.getPrice(),
  ...
  public void bidirectionalStreamingGetListsStockQuotes() throws InterruptedException {
    ...
    logger.info(
        "#######START EXAMPLE#######: BidirectionalStreaming - getListsStockQuotes from list of stocks");
    final CountDownLatch finishLatch = new CountDownLatch(1);
    StreamObserver<StockQuote> responseObserver =
        new StreamObserver<StockQuote>() {
          @Override
          ...
          public void onError(Throwable t) {
            logger.warn(
                "bidirectionalStreamingGetListsStockQuotes Failed: {0}", Status.fromThrowable(t));
            finishLatch.countDown();
          }
        };
    StreamObserver<Stock> requestObserver =
