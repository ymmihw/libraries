...
public class CombiningPublishersIntegrationTest {
  ...
  private static Flux<Integer> evenNumbers = Flux.range(min, max).filter(x -> x % 2 == 0).log();
  private static Flux<Integer> oddNumbers = Flux.range(min, max).filter(x -> x % 2 > 0).log();

  @Test
  public void givenFluxes_whenMergeDelayErrorIsInvoked_thenMergeDelayError() {
    Flux<Integer> fluxOfIntegers =
        Flux.mergeDelayError(1, Flux.just(-1, null).log(), evenNumbers, oddNumbers);

    StepVerifier.create(fluxOfIntegers).expectNext(-1).expectNext(2).expectNext(4).expectNext(1)
        .expectNext(3).expectNext(5).expectError().verify();
  }
  ...
  public void givenFluxes_whenMergeWithDelayedElementsIsInvoked_thenMergeWithDelayedElements() {
    Flux<Integer> fluxOfIntegers = Flux.merge(evenNumbers.delayElements(Duration.ofMillis(2000L)),
        oddNumbers.delayElements(Duration.ofMillis(1000L)));

    StepVerifier.create(fluxOfIntegers).expectNext(1).expectNext(2).expectNext(3).expectNext(5)
        .expectNext(4).expectComplete().verify();
  }


  @Test
  public void givenFluxes_whenConcatIsInvoked_thenConcat() {
    Flux<Integer> fluxOfIntegers = Flux.concat(evenNumbers.delayElements(Duration.ofMillis(2000L)),
        oddNumbers.delayElements(Duration.ofMillis(1000L)));

    StepVerifier.create(fluxOfIntegers).expectNext(2).expectNext(4).expectNext(1).expectNext(3)
        .expectNext(5).expectComplete().verify();
  }

  @Test
  public void givenFluxes_whenConcatIsInvoked_thenConcat2() {
  ...
  public void givenFluxes_whenCombineLatestIsInvoked_thenCombineLatest() {
    Flux<Integer> fluxOfIntegers = Flux.combineLatest(
        evenNumbers.delayElements(Duration.ofMillis(1000L)), oddNumbers, (a, b) -> a + b);

    StepVerifier.create(fluxOfIntegers).expectNext(7).expectNext(9).expectComplete().verify();
  }
  ...
  public void givenFluxes_whenCombineLatestIsInvoked_thenCombineLatest1() {
    StepVerifier.create(Flux.combineLatest(obj -> (int) obj[1], evenNumbers, oddNumbers))
        .expectNext(1).expectNext(3).expectNext(5).verifyComplete();
  }
