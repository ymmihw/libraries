...
public class ReactorIntegrationTest {
  ...
  public void givenFlux_whenZipping_thenCombine() {
    ...
    assertThat(elements).containsExactly("First Flux: 0, Second Flux: 2",
        "First Flux: 1, Second Flux: 4", "First Flux: 2, Second Flux: 6",
  ...
  public void givenFlux_whenApplyingBackPressure_thenPushElementsInBatches()
    ...
    Flux.just(1, 2, 3, 4).log().map(i -> i * 2).onBackpressureBuffer()
  ...
  @Test
  public void givenFlux_whenInParallel_thenSubscribeInDifferentThreads()
      throws InterruptedException {
    List<String> threadNames = new ArrayList<>();

    Flux.just(1, 2, 3, 4).log().map(i -> Thread.currentThread().getName())
        .subscribeOn(Schedulers.parallel()).subscribe(threadNames::add);
