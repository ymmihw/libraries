...
public class AsyncHttpClientLiveTest {
  ...
  public void givenWebSocketClient_tryToConnect() {
    ...
    try {
      WEBSOCKET_CLIENT = Dsl.asyncHttpClient().prepareGet("ws://localhost:5590/websocket")
          .addHeader("header_name", "header_value").addQueryParam("key", "value")
          .setRequestTimeout(5000).execute(wsHandler).get();
