package com.ymmihw.libraries.jgroups;
...
public class JGroupsMessenger extends ReceiverAdapter {
  ...
  private void processCommandline(String[] args) throws ParseException {
    ...
    options.addOption("u", "user", true, "User name").addOption("c", "cluster", true,
        "Cluster name");
    ...
    while (running) {
      try {
        ...
        if (destinationName.equals("x")) {
          ...
          destination =
              getAddress(destinationName).orElseThrow(() -> new Exception("Destination not found"));
        }
  ...
  public void receive(Message message) {
    ...
    String line = "Message received from: " + message.getSrc() + " to: " + message.getDest()
        + " -> " + message.getObject();
  ...
  private Optional<Address> getAddress(String name) {
    ...
    return view.getMembers().stream().filter(address -> name.equals(address.toString()))
        .findFirst();
  }
