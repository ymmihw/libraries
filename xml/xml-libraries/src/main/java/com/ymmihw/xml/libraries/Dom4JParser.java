...
public class Dom4JParser {
  ...
  public Node getElementsListByTitle(String name) {
    try {
      ...
      Document document = reader.read(file);
      List<Node> elements =
          document.selectNodes("//tutorial[descendant::title[text()=" + "'" + name + "'" + "]]");
      return elements.get(0);
    } catch (DocumentException e) {
      e.printStackTrace();
      return null;
  ...
  public void generateModifiedDocument() {
    try {
      ...
      XMLWriter writer =
          new XMLWriter(new FileWriter(new File("target/example_dom4j_updated.xml")));
  ...
  public void generateNewDocument() {
    try {
      ...
      XMLWriter writer =
          new XMLWriter(new FileWriter(new File("target/example_dom4j_new.xml")), format);
      writer.write(document);
      writer.close();
    } catch (IOException e) {
      // TODO Auto-generated catch block
